...
:root {
  ...
  /* Поверхность карточек: матовый металл (сталь с бликом) */
  --surface:        linear-gradient(180deg, rgba(255,255,255,0.06) 0%, rgba(255,255,255,0) 30%, rgba(0,0,0,0.15) 100%), #1D1F22;
  --surface-border: rgba(232,230,224,0.10);
  --surface-sheen:  inset 0 1px 0 rgba(255,255,255,0.12);
}
```
...
### 1.1 Металлическая поверхность карточек

Все карточки и панели (`.card`, `.products .card`, `.grid-item`, `.info`, панель формы заявки
`#contactForm`) используют одну поверхность — токены `--surface`, `--surface-border`,
`--surface-sheen`; инпуты внутри панелей остаются на `--bg` (поля, «врезанные в металл»).
